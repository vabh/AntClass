...
public class MainServer {
	...
	// TODO: declare the member variable for array of objects in order to have
	...
	private final Board board = new Board(boardSize, boardSize,
			TYPES_OF_OBJECTS, MAX_HEAP_SIZE);
	...
	// TODO: use this function for processing some ants on the 1st remote
	// client/processor
	// (Hint: pass ants and grid object as parameters to the remote function)
	public synchronized void processAntsOnClient(String ipAddress,
			int portNumber, Ant ant, Board board) {
		try {
			Registry rmiRegistry = LocateRegistry.getRegistry(ipAddress,
					portNumber); // fire to localhost port 1099
			IMessage rmiMessage = (IMessage) rmiRegistry
					.lookup(IMessage.messageTag); // search for service with the
													// right message
			rmiMessage.remoteAntProcessor(ant, board); // call server's method
														// with parameters
			System.out
					.println("IMessage Sent with param 1 to remoteAntProcessor()");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
		...
		for (int i = 0; i < mainServer.NUM_OF_LOOPS; ++i) { // each step of this
			...
			Thread processor1 = new Thread() {
				public void run() {
					...
					for (int i = 0; i < mainServer.NUM_OF_ANTS
							/ mainServer.NUMBER_OF_PROCESSORS; ++i) {
						mainServer.processAntsOnClient("127.0.0.1", 1099,
								mainServer.ants[i], mainServer.board);
					}
				}
			};
			processor1.start();
